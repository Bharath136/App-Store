...
class AppStore extends Component {
  state = {activeCategory: tabsList[0].tabId, searchInput: ''}
  ...
  getActiveCategory = () => {
    ...
    const activeCategoryAppList = appsList.filter(
      eachApp => eachApp.category === activeCategory,
    )

    return activeCategoryAppList
  }
  ...
  render() {
    const activeCategoryAppList = this.getActiveCategory()
    const {activeCategory, searchInput} = this.state
    const searchedAppList = activeCategoryAppList.filter(eachApp =>
        ...
        <ul className="tabs-list">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={eachTab.tabId === activeCategory}
            />
          ))}
        </ul>
